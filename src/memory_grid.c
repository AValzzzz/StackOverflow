...
{
    int wildCap = grid->glitchExploitLevel;
    int wildUsed = 0;
    int refSuitGroup = -1, refSuitExact = -1, refRank = -1;
    bool sameSuit = true, sameSuitExact = true, sameRank = true;
    int nonWildValues[LINE_LEN_MAX];
    int nonWildCount = 0;

    for (int i = 0; i < length; i++)
    {
        const Card *c = &grid->cards[line[i][0]][line[i][1]];
        if (c->isGlitched && wildUsed < wildCap) { wildUsed++; continue; }

        int sg = suitGroup(grid, c->suit);
        if (refSuitGroup < 0) refSuitGroup = sg;
        else if (sg != refSuitGroup) sameSuit = false;

        if (refSuitExact < 0) refSuitExact = (int)c->suit;
        else if ((int)c->suit != refSuitExact) sameSuitExact = false;
    ...
    {
        int v = nonWildValues[i], j = i - 1;
        while (j >= 0 && nonWildValues[j] > v) { nonWildValues[j + 1] = nonWildValues[j]; j--; }
        nonWildValues[j + 1] = v;
    }
    bool isStraight = true;
    for (int i = 1; i < nonWildCount; i++)
        if (nonWildValues[i] == nonWildValues[i - 1]) { isStraight = false; break; }
    if (isStraight && nonWildCount > 0 && (nonWildValues[nonWildCount - 1] - nonWildValues[0]) > length - 1)
        isStraight = false;
...
static bool jitSpreadEligible(const MemoryGrid *grid, int row, int col)
{
    if (row < 0 || col < 0 || row >= grid->size || col >= grid->size) return false;
    const Card *c = &grid->cards[row][col];
    return !c->isLocked && !c->isGlitched;
}

static int jitCompilerSpreadTick(MemoryGrid *grid)
{
    if (grid->jitCompilerLevel <= 0) return 0;

    int sources[GRID_SIZE_MAX * GRID_SIZE_MAX][2];
    int sourceCount = 0;
    for (int row = 0; row < grid->size; row++)
        for (int col = 0; col < grid->size; col++)
            if (grid->cards[row][col].isGlitched)
            {
                sources[sourceCount][0] = row;
                sources[sourceCount][1] = col;
                sourceCount++;
            }
    ...
    int spread = 0;
    for (int attempt = 0; attempt < grid->jitCompilerLevel; attempt++)
    {
        int *src = sources[rand() % sourceCount];
        int row = src[0], col = src[1];

        int candidates[4][2], candidateCount = 0;
        for (int d = 0; d < 4; d++)
        {
            int nr = row + dr[d], nc = col + dc[d];
            if (jitSpreadEligible(grid, nr, nc))
            {
                candidates[candidateCount][0] = nr;
                candidates[candidateCount][1] = nc;
                candidateCount++;
            }
        }
        if (candidateCount == 0) continue;

        int *target = candidates[rand() % candidateCount];
        int before = cellValue(grid, target[0], target[1]);
        card_markGlitched(&grid->cards[target[0]][target[1]]);
        grid->stackScore += cellValue(grid, target[0], target[1]) - before;
        spread++;
    }
    return spread;
}

int memorygrid_tickTurn(MemoryGrid *grid)
{
    if (grid->diagonalModeFrozenTurns > 0) grid->diagonalModeFrozenTurns--;
    return jitCompilerSpreadTick(grid);
}
...
static bool fairDealFindSwap(const MemoryGrid *grid, const Deck *deck, bool allowLocked,
                              int *outRow, int *outCol)
{
    ...
    *outCol = -1;
    for (int r = 0; r < grid->size; r++)
        ...
        {
            if (!allowLocked && grid->cards[r][c].isLocked) continue;
            int value = card_getEffectiveValue(&grid->cards[r][c]);
            if (value <= bestValue) continue;
            if (!deck_hasCardBelow(deck, value)) continue;
            bestValue = value;
            *outRow = r;
            *outCol = c;
        }
...
static void fairDealPass(MemoryGrid *grid, Deck *deck, int stackLimit, bool allowLocked,
                          int maxAttempts)
{
    for (int attempt = 0; attempt < maxAttempts && grid->stackScore > stackLimit; attempt++)
    {
        ...
        int row, col;
        if (!fairDealFindSwap(grid, deck, allowLocked, &row, &col)) break;

        int bestValue = card_getEffectiveValue(&grid->cards[row][col]);
        deck_injectCard(deck, grid->cards[row][col]);
        memorygrid_placeCard(grid, row, col, deck_drawCardBelow(deck, bestValue));
    }
}

static void forceDealPass(MemoryGrid *grid, Deck *deck, int stackLimit, int maxAttempts)
{
    for (int attempt = 0; attempt < maxAttempts && grid->stackScore > stackLimit; attempt++)
    {
        ...
        for (int r = 0; r < grid->size; r++)
            for (int c = 0; c < grid->size; c++)
            {
                int value = cellValue(grid, r, c);
                if (value > bestValue) { bestValue = value; bestRow = r; bestCol = c; }
            }
        if (bestRow < 0 || bestValue <= 0) break;

        Card replacement = deck_drawLowestValueCard(deck);
        if (card_getEffectiveValue(&replacement) >= bestValue)
        {
            deck_injectCard(deck, replacement);
            break;
        }

        deck_injectCard(deck, grid->cards[bestRow][bestCol]);
        memorygrid_placeCard(grid, bestRow, bestCol, replacement);
    }
}
...
{
    fairDealPass(grid, deck, stackLimit, false, maxAttempts);

    if (grid->stackScore > stackLimit)
        fairDealPass(grid, deck, stackLimit, true, maxAttempts);

    if (grid->stackScore > stackLimit)
        forceDealPass(grid, deck, stackLimit, maxAttempts);

    if (grid->stackScore > stackLimit)
        grid->stackScore = stackLimit;
