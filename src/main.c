...
#define EXTRA_MODULE_SLOT_PRICE 40

#define RAM_UPGRADE_BASE_PRICE 15
#define RAM_UPGRADE_PRICE_STEP 10
#define RAM_UPGRADE_STACK_BONUS 3
...
static const Color COLOR_GOLD = {255, 178, 40, 255};
...
typedef enum ActiveModal {
  MODAL_NONE,
  MODAL_SETTINGS,
  MODAL_CHESS_TUTORIAL,
  MODAL_CONDITION_TUTORIAL,
  MODAL_HELP,
  MODAL_DECK_POPUP,
  MODAL_DECK_EDIT,
  MODAL_PAUSE,
  MODAL_SHOP_TUTORIAL,
  MODAL_SHOP_SWAP_PROMPT
} ActiveModal;
...
static int effectiveStackLimit(const Game *g) {
  return g->roundCfg.stackLimit + g->ramUpgradesBought * RAM_UPGRADE_STACK_BONUS;
}

static int ramUpgradePrice(const Game *g) {
  return RAM_UPGRADE_BASE_PRICE + g->ramUpgradesBought * RAM_UPGRADE_PRICE_STEP;
}
...
static void clampCombinedStackToLimit(Game *g) {
  int excess = g->grid.stackScore - effectiveStackLimit(g);
  if (excess <= 0)
    return;
...
static Rectangle shopExpandSlotBtnRect(void) {
  return (Rectangle){SCREEN_WIDTH / 2.0f + 280, 610, 170, 50};
}

static Rectangle shopRamUpgradeBtnRect(void) {
  return (Rectangle){SCREEN_WIDTH / 2.0f - 450, 610, 170, 50};
}
...
static void deckEditRemoveAt(Game *g, Card card, int compositionIndex,
  ...
  if (compositionIndex >= baseCount) {
    ...
    if (i < 0 || i >= g->boughtCardCount) return;
...
static void drawItemTooltip(const ShopItemInfo *info, Rectangle anchor) {
  ...
  float x = anchor.x;
  float y = anchor.y + anchor.height + 6.0f;
  if (x + width > SCREEN_WIDTH - 4.0f)
    x = SCREEN_WIDTH - width - 4.0f;
  if (x < 4.0f)
    x = 4.0f;
  if (y + height > SCREEN_HEIGHT - 4.0f)
    y = anchor.y - height - 6.0f;
  if (y < 4.0f)
    y = 4.0f;

  Rectangle tip = {x, y, width, height};
  DrawRectangleRec(tip, COLOR_PANEL);
  DrawRectangleLinesEx(tip, 2, COLOR_PROMPT);
  DrawText(tr(info->name), (int)(tip.x + padding), (int)(tip.y + 6), 15,
           RAYWHITE);
  drawTextWrappedClipped(tr(info->description), tip.x + padding, tip.y + 26,
                         width - 2 * padding, height - 32, 13, 16, LIGHTGRAY);
}
...
static void resolveGridChange(Game *g) {
  ...
  g->cascade.anyStraightFlush = false;
}
...
static void finishCascade(Game *g) {
  ...
  bool isWipe = g->cascade.anyStraightFlush;
  ...
  if (anyCombo) {
    ...
    g->scorePopup.isWipe = isWipe;
    g->scorePopup.tier = isWipe ? 4 : comboTierForStreak(streak);
    ...
    if (g->scorePopup.tier >= 4)
      triggerShake(g, SHAKE_DURATION_COMBO * 2.2f, 14.0f);
  }
  ...
  if (combinedStackScore(g) > effectiveStackLimit(g)) {
    if (anyCombo) {
...
static void pulseModuleSlot(Game *g, ShopItemId id) {
  int slot = moduleIdSlotIndex(&g->inventory, id);
  if (slot < 0)
    return;
  g->moduleSlotPulse[slot] = MODULE_SLOT_PULSE_DURATION;
...
static void applyCascadeWave(Game *g) {
  ...
  if (combo.straightFlushMatches > 0)
    g->cascade.anyStraightFlush = true;
  g->cascade.wave++;
  g->cascade.revealing = false;
...
static void resolveTurnEnd(Game *g) {
  if (memorygrid_tickTurn(&g->grid) > 0)
    pulseModuleSlot(g, ITEM_JIT_COMPILER);
...
static void tryTriggerInterrupt(Game *g) {
  ...
  if (!tutorial_hasSeenConditionIntro()) {
    g->activeModal = MODAL_CONDITION_TUTORIAL;
...
static void enterShop(Game *g) {
  ...
  g->activeModal = MODAL_NONE;

  if (!tutorial_hasSeenShopIntro()) {
    g->activeModal = MODAL_SHOP_TUTORIAL;
...
static void writeProgressSave(const Game *g) {
  ...
  save.gold = g->gold;
  save.ramUpgradesBought = g->ramUpgradesBought;
...
static void startNewRound(Game *g) {
  ...
  if (g->startingClass == CLASS_COMPILER &&
      g->deck.count > 0) {
  ...
  if (g->roundCfg.chessUnlocked && !tutorial_hasSeenChessIntro()) {
    g->activeModal = MODAL_CHESS_TUTORIAL;
...
static bool rollModuleChoiceOffer(Game *g) {
  ...
  for (int i = 0; i < g->inventory.unlockedModuleSlots; i++)
...
static void resumeFromSave(Game *g, const GameSave *save) {
  ...
  g->gold = save->gold;
  g->ramUpgradesBought = save->ramUpgradesBought;
  ...
  while (!WindowShouldClose() && !game.wantsQuit) {
    ...
    for (int i = 0; i < HAND_SIZE; i++) {
      bool hovered = g->phase == PHASE_PLAYING && !g->isDragging &&
                     g->activeModal == MODAL_NONE && g->hand.occupied[i] &&
    ...
    if (g->phase == PHASE_LANGUAGE_SELECT) {
    ...
    } else if (g->activeModal == MODAL_SETTINGS) {
      if (IsKeyPressed(KEY_ESCAPE)) {
        g->activeModal = g->settingsOpenedFromPause ? MODAL_PAUSE : MODAL_NONE;
        audio_playUiClick();
    ...
    } else if (g->activeModal == MODAL_CHESS_TUTORIAL) {
    ...
    } else if (g->activeModal == MODAL_CONDITION_TUTORIAL) {
      if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) || IsKeyPressed(KEY_SPACE) ||
          IsKeyPressed(KEY_ENTER)) {
        audio_playUiClick();
    ...
    } else if (g->activeModal == MODAL_HELP) {
      ...
      if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) || IsKeyPressed(KEY_ESCAPE) ||
        ...
        g->activeModal = MODAL_NONE;
               ...
               g->activeModal == MODAL_NONE && !g->cascade.active &&
               (IsKeyPressed(KEY_H) ||
                (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) &&
                 CheckCollisionPointRec(GetMousePosition(),
                                        hudHelpButtonRect())))) {
      g->activeModal = MODAL_HELP;
      g->helpScrollY = 0.0f;
    } else if (g->activeModal == MODAL_DECK_POPUP) {
      if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) || IsKeyPressed(KEY_ESCAPE))
        g->activeModal = MODAL_NONE;
    } else if (g->activeModal == MODAL_DECK_EDIT) {
      if (IsKeyPressed(KEY_ESCAPE))
        g->activeModal = MODAL_NONE;
      else if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        Vector2 mouse = GetMousePosition();
    ...
    } else if (g->activeModal == MODAL_PAUSE) {
      if (IsKeyPressed(KEY_ESCAPE)) {
        g->activeModal = MODAL_NONE;
        audio_playUiClick();
      } else if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
    ...
    } else if (g->phase == PHASE_SHOP && g->activeModal == MODAL_SHOP_TUTORIAL) {
      if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) || IsKeyPressed(KEY_SPACE) ||
          IsKeyPressed(KEY_ENTER)) {
        audio_playUiClick();
        g->shopTutorialStep++;
        if (g->shopTutorialStep >= SHOP_TUTORIAL_STEP_COUNT) {
          ...
          tutorial_markShopIntroSeen();
               ...
               g->activeModal == MODAL_SHOP_SWAP_PROMPT) {
      if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        Vector2 mouse = GetMousePosition();
        int slotCount = g->shopSwapIsModule ? g->inventory.unlockedModuleSlots
                                            : SCRIPT_SLOTS;
      ...
      if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        ...
        if (CheckCollisionPointRec(mouse, editDeckBtn))
          g->activeModal = MODAL_DECK_EDIT;

        if (!inventory_moduleSlotsMaxed(&g->inventory) &&
            CheckCollisionPointRec(mouse, shopExpandSlotBtnRect())) {
          if (g->gold < EXTRA_MODULE_SLOT_PRICE) {
            audio_playDeny();
            setStatus(g, TextFormat(tr(STR_STATUS_GOLD_SHORT_EXPAND),
                                    EXTRA_MODULE_SLOT_PRICE));
          } else if (inventory_buyExtraModuleSlot(&g->inventory)) {
            g->gold -= EXTRA_MODULE_SLOT_PRICE;
            audio_playShopBuy();
          }
        }

        if (CheckCollisionPointRec(mouse, shopRamUpgradeBtnRect())) {
          int price = ramUpgradePrice(g);
          if (g->gold < price) {
            audio_playDeny();
            setStatus(g, TextFormat(tr(STR_STATUS_GOLD_SHORT_RAM), price));
          } else {
            g->gold -= price;
            g->ramUpgradesBought++;
            audio_playShopBuy();
      ...
      if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        ...
        if (!handled && CheckCollisionPointRec(mouse, deckStackRect())) {
          g->activeModal = MODAL_DECK_POPUP;
          handled = true;
        }
    ...
    if (g->phase == PHASE_MAIN_MENU) {
      ...
      bool slotsMaxed = inventory_moduleSlotsMaxed(&g->inventory);
      bool canExpandSlot = !slotsMaxed && g->gold >= EXTRA_MODULE_SLOT_PRICE;
      drawButton(shopExpandSlotBtnRect(),
                 slotsMaxed ? tr(STR_TAG_SLOTS_MAXED)
                            : TextFormat(tr(STR_BTN_EXPAND_SLOT),
                                        EXTRA_MODULE_SLOT_PRICE),
                 COLOR_GOLD, canExpandSlot, false);

      int ramPrice = ramUpgradePrice(g);
      bool canBuyRam = g->gold >= ramPrice;
      Rectangle ramBtn = shopRamUpgradeBtnRect();
      drawButton(ramBtn, TextFormat(tr(STR_BTN_RAM_UPGRADE), ramPrice),
                 COLOR_GOLD, canBuyRam, false);
      if (CheckCollisionPointRec(GetMousePosition(), ramBtn))
        drawTextCentered(
            TextFormat(tr(STR_RAM_UPGRADE_HINT), RAM_UPGRADE_STACK_BONUS,
                       g->ramUpgradesBought * RAM_UPGRADE_STACK_BONUS),
            ramBtn.x + ramBtn.width / 2.0f, ramBtn.y - 18, 13, LIGHTGRAY);

      if (g->statusMessageTimer > 0.0f)
        drawTextCentered(g->statusMessage, SCREEN_WIDTH / 2.0f, 590, 16,
                         COLOR_DANGER);

      if (g->activeModal == MODAL_SHOP_SWAP_PROMPT) {
      ...
      if (g->activeModal == MODAL_SHOP_TUTORIAL)
      ...
      float stackRatio =
          (float)combinedStackScore(g) / (float)effectiveStackLimit(g);
      float gaugeFill =
...

      {
        ...
        if (hoveredId != (ShopItemId)NO_ITEM)
          drawItemTooltip(shop_getItemInfo(hoveredId), hoveredSlot);
      }
      ...
      if (g->scorePopup.active) {
        ...
        if (g->scorePopup.tier > 0) {
          StrId tierStr;
          Color tierColor;
          int tierBaseSize;
          switch (g->scorePopup.isWipe ? 5 : g->scorePopup.tier) {
          case 1:
            tierStr = STR_COMBO_TIER_1;
            tierColor = COLOR_GLITCH;
            tierBaseSize = 24;
            break;
          case 2:
            tierStr = STR_COMBO_TIER_2;
            tierColor = COLOR_PROMPT;
            tierBaseSize = 29;
            break;
          case 3:
            tierStr = STR_COMBO_TIER_3;
            tierColor = COLOR_GOLD;
            tierBaseSize = 34;
            break;
          case 5:
            tierStr = STR_COMBO_TIER_WIPE;
            tierColor = COLOR_DANGER;
            tierBaseSize = 44;
            break;
          default:
            tierStr = STR_COMBO_TIER_4;
            tierColor = COLOR_MULT;
            tierBaseSize = 44;
            break;
          }
          float tierScale = easeOutBack(fminf(t / 0.35f, 1.0f));
          if (tierScale < 0.0f)
            tierScale = 0.0f;
          int tierSize = (int)(tierBaseSize * tierScale);
          float tierY = multY - 30.0f;
          if (g->scorePopup.tier >= 3 || g->scorePopup.isWipe) {
            float pulse = 0.4f + 0.2f * sinf(g->scorePopup.elapsed * 14.0f);
            drawGlowEx((Rectangle){center.x - 130, tierY - 6, 260, 34},
                       tierColor, pulse * alpha, 3, 8.0f);
          }
          drawTextCentered(tr(tierStr), center.x, tierY, tierSize,
                           Fade(tierColor, alpha));
        }
      }
    }
    ...
    if (g->activeModal == MODAL_DECK_POPUP) {
    ...
    if (g->activeModal == MODAL_DECK_EDIT) {
      Card composition[DECK_MAX_SIZE];
      int n = buildFullDeckComposition(g, composition);
      int cost = deckEditCost(g);
    ...
    if (g->phase == PHASE_GAME_OVER) {
      ...
      case REASON_CRASH:
        ...
        line2 = TextFormat(tr(STR_GAMEOVER_CRASH_LINE), combinedStackScore(g),
                           g->roundNumber, effectiveStackLimit(g));
    ...
    if (g->activeModal == MODAL_SETTINGS) {
      DrawRectangle(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, (Color){0, 0, 0, 210});
