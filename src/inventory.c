...
void inventory_init(Inventory *inv)
{
    ...
    inv->unlockedModuleSlots = MODULE_SLOTS_BASE;
...
bool inventory_buyModule(Inventory *inv, ShopItemId id)
{
    ...
    for (int i = 0; i < inv->unlockedModuleSlots; i++)
    {
    ...
    int slot = firstFree(inv->modules, inv->unlockedModuleSlots);
...
bool inventory_mergeRedundantColor(Inventory *inv)
{
    ...
    if (mergeSlot < 0) mergeSlot = firstFree(inv->modules, inv->unlockedModuleSlots);
...
{
    return firstFree(inv->modules, inv->unlockedModuleSlots) < 0;
}
...
bool inventory_moduleSlotsMaxed(const Inventory *inv)
{
    return inv->unlockedModuleSlots >= MODULE_SLOTS;
}

bool inventory_buyExtraModuleSlot(Inventory *inv)
{
    if (inventory_moduleSlotsMaxed(inv)) return false;
    inv->unlockedModuleSlots++;
    return true;
}
