...
static Sound g_victoryFanfare;
static Sound g_chessWin;
static Sound g_chessLose;
static Music g_bgm;
...
static Sound synthVictoryFanfare(void)
{
    const float noteDuration = 0.14f;
    const float freqs[4] = { 523.25f, 659.25f, 783.99f, 1046.50f };
    int perNote = (int)(noteDuration * SYNTH_SAMPLE_RATE);
    int frameCount = perNote * 4;
    short *samples = malloc(sizeof(short) * (size_t)frameCount);

    float phase = 0.0f;
    for (int i = 0; i < frameCount; i++)
    {
        int noteIndex = i / perNote;
        if (noteIndex > 3) noteIndex = 3;
        phase += freqs[noteIndex] / SYNTH_SAMPLE_RATE;
        float localT = (float)(i % perNote) / (float)perNote;
        float envelope = sinf(PI * localT);
        float sample = sinf(2.0f * PI * phase) * envelope * 0.6f;
        samples[i] = (short)(sample * 32000.0f);
    }
...
static Sound synthChessWinStinger(void)
{
    const float noteDuration = 0.1f;
    const float freqs[3] = { 440.0f, 554.37f, 880.0f };
    int perNote = (int)(noteDuration * SYNTH_SAMPLE_RATE);
    int frameCount = perNote * 3;
    short *samples = malloc(sizeof(short) * (size_t)frameCount);
...
static Sound synthChessLoseStinger(void)
{
    ...
    {
        float t = (float)i / (float)frameCount;
        float freq = 220.0f - 140.0f * t;
        phase += freq / SYNTH_SAMPLE_RATE;
        float square = (sinf(2.0f * PI * phase) >= 0.0f) ? 1.0f : -1.0f;
        float envelope = powf(1.0f - t, 1.8f);
        float sample = square * envelope * 0.45f;
        samples[i] = (short)(sample * 32000.0f);
    }
...
void audio_loadAll(void)
{
    ...
    g_victoryFanfare = synthVictoryFanfare();
    g_chessWin = synthChessWinStinger();
    g_chessLose = synthChessLoseStinger();
...
void audio_unloadAll(void)
{
    ...
    UnloadSound(g_victoryFanfare);
    UnloadSound(g_chessWin);
    UnloadSound(g_chessLose);
...
void audio_playVictoryFanfare(void) { PlaySound(g_victoryFanfare); }
void audio_playChessWin(void) { PlaySound(g_chessWin); }
void audio_playChessLose(void) { PlaySound(g_chessLose); }
...
void audio_setSfxVolume(float volume)
{
    ...
    SetSoundVolume(g_victoryFanfare, volume);
    SetSoundVolume(g_chessWin, volume);
    SetSoundVolume(g_chessLose, volume);
}
